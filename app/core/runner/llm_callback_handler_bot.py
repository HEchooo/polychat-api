...
class LLMCallbackHandlerBot:
    """
    Bot-specific LLM chat callback handler, only removes prefixes without JSON structure
    """
    ...
    def __init__(
        ...
        # Special tags that only need prefix removal
    ...
    def _process_prefix_buffer(self, incoming_text: str) -> str:
        """Process and remove prefix codes from incoming text"""
    ...
    def handle_llm_response(
        ...
        if self.remain_text:
            if self._two_phase_detected:
                ...
                logging.info("Two-phase detected, emit with prefix removed")
            else:
